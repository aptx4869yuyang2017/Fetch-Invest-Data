...
class HKConnectorFinancialReportFetcher:
    """港股通财务报表数据获取器，用于批量获取港股通财务报表数据"""
    ...
    def __init__(self, provider: Optional[Union[FinancialReportProvider, str]] = None):
        ...
        self.available_providers = {
            'akshare': HKConnectorFinancialReportProvider,
            # 可以在这里添加其他数据提供者
        }
    ...
    def fetch_balance_sheet(self, symbol: str) -> pd.DataFrame:
        ...
        try:
            ...
            if df.empty:
                self.logger.warning(f"未获取到港股 {symbol} 的资产负债表数据")
            else:
                self.logger.debug(f"成功获取港股 {symbol} 的资产负债表数据，共 {len(df)} 条记录")
    ...
    def fetch_income_statement(self, symbol: str) -> pd.DataFrame:
        ...
        try:
            ...
            if df.empty:
                self.logger.warning(f"未获取到港股 {symbol} 的利润表数据")
            else:
                self.logger.debug(f"成功获取港股 {symbol} 的利润表数据，共 {len(df)} 条记录")
    ...
    def fetch_cash_flow_statement(self, symbol: str) -> pd.DataFrame:
        ...
        try:
            ...
            if df.empty:
                self.logger.warning(f"未获取到港股 {symbol} 的现金流量表数据")
            else:
                self.logger.debug(f"成功获取港股 {symbol} 的现金流量表数据，共 {len(df)} 条记录")
    ...
    def fetch_multiple_cash_flow_statements(self, symbols: List[str], max_workers: int = 5,
        ...
        Args:
            symbols: 港股股票代码列表，格式为 ["00700", "00001"]
            max_workers: 最大线程数，默认为 5
            delay: 每个请求之间的延迟时间(秒)，避免频繁请求被限制，默认0.5秒
            merge_results: 是否合并结果为一个DataFrame，默认为True

        Returns:
            如果merge_results为True，返回合并后的DataFrame，包含所有港股的现金流量表数据，
            并添加'symbol'列标识股票代码；
            否则返回包含多个港股现金流量表的字典，键为股票代码，值为对应的DataFrame
        """
        self.logger.info(f"开始批量获取 {len(symbols)} 只港股的现金流量表数据")
    ...
    def fetch_multiple_balance_sheets(self, symbols: List[str], max_workers: int = 5,
        ...
        Returns:
            如果merge_results为True，返回合并后的DataFrame，包含所有港股的资产负债表数据，
            并添加'symbol'列标识股票代码；
            否则返回包含多个港股资产负债表的字典，键为股票代码，值为对应的DataFrame
        """
        self.logger.info(f"开始批量获取 {len(symbols)} 只港股的资产负债表数据")
    ...
    def fetch_multiple_income_statements(self, symbols: List[str], max_workers: int = 5,
        ...
        Returns:
            如果merge_results为True，返回合并后的DataFrame，包含所有港股的利润表数据，
            并添加'symbol'列标识股票代码；
            否则返回包含多个港股利润表的字典，键为股票代码，值为对应的DataFrame
        """
        self.logger.info(f"开始批量获取 {len(symbols)} 只港股的利润表数据")
